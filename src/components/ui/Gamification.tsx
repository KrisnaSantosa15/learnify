  ...
  return (
    <div 
      className={`bg-white/5 backdrop-blur-sm border border-white/10 rounded-xl p-5 relative overflow-hidden transition-all duration-300 ${
        earned ? `hover:border-${rarityMap[rarity].text}/50 hover:shadow-lg hover:${rarityMap[rarity].shadow}` : 'grayscale-[70%] opacity-70'
      }`}
    >
      {/* Rarity indicator */}
      <div className={`absolute top-3 right-3 px-2 py-1 text-xs font-medium uppercase rounded-full ${rarityMap[rarity].badgeBg} ${rarityMap[rarity].badgeText}`}>
        {rarity}
      </div>
      ...
      <div className="text-center">
        ...
        >
          <div className={`text-3xl ${earned ? `text-[${rarityMap[rarity].text}]` : 'text-gray-500'}`}>
            {icon}
          </div>
        </div>
        <h3 className="text-lg font-bold text-white mb-1">{title}</h3>
        <p className="text-[#a8b1c2] text-sm mb-3">{description}</p>
        <div className={`inline-block px-2 py-1 text-xs font-medium rounded-full ${rarityMap[rarity].badgeBg} ${rarityMap[rarity].badgeText}`}>+{xpReward} XP</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div 
      className={`flex items-center p-3 rounded-lg transition-all ${
        isUser ? 'bg-[#8e5ff5]/10 border border-[#8e5ff5]/30' : 'hover:bg-white/5'
      }`}
    >
      <div className="text-lg font-medium w-6 text-center text-white">{rank}</div>
      ...
      <div className="ml-4 flex-1">
        <div className={`font-medium ${isUser ? 'text-[#8e5ff5]' : 'text-white'}`}>
          {name} {isUser && <span className="text-xs ml-1 opacity-70">(you)</span>}
        </div>
      </div>
      
      <div className={`font-bold ${isUser ? 'text-[#8e5ff5]' : 'text-[#a8b1c2]'}`}>
    ...
    <div className="space-y-2">
      {(label || showPercentage) && (
        <div className="flex justify-between items-center">
          ...
          {showPercentage && <span className={`${colorMap[color].text} text-sm font-medium`}>{value}%</span>}
        </div>
      )}
      <div className={`w-full ${height} bg-[#1c2841] rounded-full overflow-hidden`}>
        <div className={`h-full ${colorMap[color].bg} rounded-full`} style={{ width: `${value}%` }}></div>
      </div>
    </div>
  );
}
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div 
          ...
          style={{ 
            background: 'radial-gradient(circle at center, rgba(142, 95, 245, 0.8), transparent 70%)',
          }}
        ></div>
        {/* Small circles */}
        <div 
          ...
          style={{ 
            background: 'radial-gradient(circle at center, rgba(255, 94, 125, 0.8), transparent 70%)',
          }}
        ></div>
      </div>
      ...
      <div className="container mx-auto px-6 lg:px-10 relative z-10">
        ...
        >
          ...
          <p className="text-lg text-[#a8b1c2]">
            Stay motivated with coding challenges, achievements, and competition.
            Turn learning to code into an addictive game that levels up your real skills.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          >
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <div className="h-8 w-8 rounded-lg bg-[#1d0b3d] flex items-center justify-center">
                    <svg className="w-5 h-5 text-[#8e5ff5]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 3.055A9.001 9.001 0 1020.945 13H11V3.055z" />
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M20.488 9H15V3.512A9.025 9.025 0 0120.488 9z" />
                    </svg>
                  </div>
                  <span className="text-white font-medium">Your Coding Stats</span>
                </div>
                <div className="flex items-center gap-2">
                  <div className="px-3 py-1 bg-[#8e5ff5]/20 text-[#8e5ff5] text-xs rounded-full">Level 8 Coder</div>
                </div>
              </div>
            </div>
            ...
            <div className="p-6">
                ...
                <div className="bg-[#131d33]/80 rounded-lg p-4">
                  <div className="grid grid-cols-7 gap-1 mb-2 text-center">
                    {['M', 'T', 'W', 'T', 'F', 'S', 'S'].map((day, i) => (
                      <div key={i} className="text-xs text-[#6a7a95]">{day}</div>
                    ))}
                  </div>
                ...
                <div className="space-y-3">
                  <div className="bg-[#131d33]/80 rounded-lg p-3 flex items-center">
                    <div className="h-8 w-8 rounded-full bg-[#28c7f9]/20 flex items-center justify-center mr-3">
                      <svg className="w-4 h-4 text-[#28c7f9]" viewBox="0 0 20 20" fill="currentColor">
                        <path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z" />
                        <path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd" />
                      </svg>
                    </div>
                    <div className="flex-1">
                      <div className="text-sm text-white">Completed JavaScript Challenge</div>
                  ...
                  <div className="bg-[#131d33]/80 rounded-lg p-3 flex items-center">
                    ...
                    <div className="flex-1">
                      <div className="text-sm text-white">Earned &quot;Bug Destroyer" Badge</div>
                      <div className="text-xs text-[#6a7a95] flex justify-between mt-1">
                        <span>Yesterday</span>
                  ...
                  <div className="bg-[#131d33]/80 rounded-lg p-3 flex items-center">
                    <div className="h-8 w-8 rounded-full bg-[#fab72b]/20 flex items-center justify-center mr-3">
                      <svg className="w-4 h-4 text-[#fab72b]" viewBox="0 0 20 20" fill="currentColor">
              ...
              <div className="text-center">
                <Link href="/profile" className="text-[#8e5ff5] text-sm hover:text-[#a47afb] transition-colors flex items-center justify-center">
          ...
          <div className="space-y-6">
            ...
            >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    <div className="h-8 w-8 rounded-lg bg-[#2d0b1e] flex items-center justify-center">
                      <svg className="w-5 h-5 text-[#ff5e7d]" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 3v4M3 5h4M6 17v4m-2-2h4m5-16l2.286 6.857L21 12l-5.714 2.143L13 21l-2.286-6.857L5 12l5.714-2.143L13 3z" />
                      </svg>
                    </div>
                    <span className="text-white font-medium">Coding Achievements</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="px-3 py-1 bg-[#ff5e7d]/20 text-[#ff5e7d] text-xs rounded-full">12 Unlocked</div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="mt-6 text-center">
                  <Link href="/achievements" className="text-[#ff5e7d] text-sm hover:text-[#ff7b8f] transition-colors flex items-center justify-center">
            ...
            >
              <div className="p-6 bg-[#131d33]">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-white font-medium">Weekly Leaderboard</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="px-3 py-1 bg-[#8e5ff5]/20 text-[#8e5ff5] text-xs rounded-full">Top 5%</div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-6">
                ...
                <div className="mt-6 text-center">
                  <Link href="/leaderboard" className="text-[#8e5ff5] text-sm hover:text-[#a47afb] transition-colors flex items-center justify-center">
        ...
        <div 
          ...
          style={{
            ...
            background: 'linear-gradient(135deg, rgba(21, 31, 56, 0.6) 0%, rgba(45, 11, 30, 0.6) 100%)'
          }}
        >
