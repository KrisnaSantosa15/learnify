...
export default function CourseProgress() {
  ...
  const [dailyProgress, setDailyProgress] = useState(45); // in minutes
  ...
  const getColorClass = (color: string) => {
    switch (color) {
      case 'green': return { bg: 'bg-[#58c896]', bgLight: 'bg-[#58c896]/20', text: 'text-[#58c896]' };
      case 'purple': return { bg: 'bg-[#8e5ff5]', bgLight: 'bg-[#8e5ff5]/20', text: 'text-[#8e5ff5]' };
      case 'blue': return { bg: 'bg-[#28c7f9]', bgLight: 'bg-[#28c7f9]/20', text: 'text-[#28c7f9]' };
      default: return { bg: 'bg-[#58c896]', bgLight: 'bg-[#58c896]/20', text: 'text-[#58c896]' };
    }
  };
      ...
      <div className="p-4 border-b border-white/5 bg-dark-200/30">
        <div className="flex justify-between items-center mb-2">
          <div>
            ...
            <p className="text-xs text-gray-400">{dailyProgress} of {dailyGoal} minutes completed</p>
          </div>
      ...
      <div className="p-5 flex-1 overflow-y-auto">
        <div className="flex flex-col space-y-4">
          {courses.map((course) => {
            ...
            return (
              <div key={course.id} className="bg-dark-200/50 rounded-lg p-4 hover:bg-dark-200 transition-colors">
                <div className="flex items-center gap-3 mb-3">
                  <div className={`h-10 w-10 rounded-lg ${colorClasses.bgLight} flex items-center justify-center text-xl`}>
                    {course.icon}
                  </div>
                  <div className="flex-1">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <p className="text-gray-500 text-xs">Last accessed: {course.lastAccessed}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={colorClasses.text + " text-xs"}>
                    {course.progress < 30 ? 'Just Started' : 
                     course.progress < 70 ? 'In Progress' : 
                     'Almost Complete'}
                  </span>
                </div>
                ...
                <div className="mt-3 flex items-center justify-between">
                  ...
                  <button className={`px-3 py-1 rounded text-xs ${colorClasses.bg} text-white`}>
                    Continue
                  </button>
                </div>
                
                {course.relatedAchievements && course.relatedAchievements.length > 0 && (
                  <div className="mt-3 border-t border-white/5 pt-2">
                    <p className="text-xs text-gray-400 mb-1.5">Related achievements:</p>
                    <div className="flex flex-wrap gap-1">
                      {course.relatedAchievements.map((achievement, index) => (
                        <span key={index} className="px-2 py-0.5 bg-dark-300/50 rounded text-xs text-gray-300">
                          {achievement}
                        </span>
                      ))}
                    </div>
          ...
          <div className="mt-2 pt-2 border-t border-white/10">
            ...
            {recommendations.map((course) => {
              const colorClasses = getColorClass(course.color);
              return (
                <div key={course.id} className="bg-dark-200/30 rounded-lg p-3 mb-3 hover:bg-dark-200/70 transition-colors flex items-center gap-3">
                  <div className={`h-9 w-9 rounded-lg ${colorClasses.bgLight} flex items-center justify-center text-lg`}>
                    {course.icon}
                  </div>
                  <div className="flex-1">
